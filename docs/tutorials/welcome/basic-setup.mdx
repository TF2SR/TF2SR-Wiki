...
In the unlikely event that you _don't_ already own Titanfall 2, it's available both on [Steam](https://store.steampowered.com/app/1237970/Titanfall\_2/) and the [EA App](https://www.ea.com/de-de/games/titanfall/titanfall-2/buy/pc). If you have the patience, it is recommended to wait for it to go on Sale as it regularly gets between [75% and 90% off on Steam](https://steamdb.info/app/1237970/).<br/>
If you have the option of picking between getting the game on PC or Console, _playing the game on PC is highly recommended_ as the PC version has access to practice tools and fixes that the Console version cannot get.

### Timers

**On PC,** the most commonly used speedrun timer is called [LiveSplit](http://livesplit.org/). It also has a web-based cross-platform version called [LiveSplit One](https://one.livesplit.org/). (Though LiveSplit is recommended since it has access to our Autosplitter that automatically removes load times for you)
...
Generally speaking, streaming your runs is a good idea since if you set it up correctly, it eliminates the tedious work of uploading your >1h videos since you can just save them directly. having a local recording running as a backup is still good practice in case you lose connection, if your system can handle it.
...
Once your Personal Best approaches 1:25:00 (Both in Any% and All Helmets) you're going to need an **input overlay.** These aren't necessary until you actually reach this milestone, but since you never know when you might break this barrier, it's best to set up an input overlay earlier rather than later. Here are a few options:

* [Nohboard](https://github.com/ThoNohT/NohBoard/releases)
* [OBS Input overlay](https://github.com/univrsal/input-overlay/releases/tag/v5.0.5)
